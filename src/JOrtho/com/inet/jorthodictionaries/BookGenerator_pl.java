...
public class BookGenerator_pl extends BookGenerator {
    ...
    boolean isValidLanguage( String word, String wikiText ) {
        if( wikiText.indexOf( "{{jzyk polski}}" ) < 0 && wikiText.indexOf( "{{Jzyk polski}}" ) < 0) {
            return false;
        }
    ...
    private void odmiana( int idx, String wikiText ) {
        // sample
        // {{odmiana}} {{M}}: {{m}} ktr|y, {{f}} ~a, {{n}} ~e; {{lm m}} ~zy, {{lm nm}} ~e; {{D}}: ~ego, ~ej, ~ego, ~ych, ~ych; {{C}}: ~emu, ~ej, ~emu, ~ym, ~ym; {{B}}: ~ego, ~, ~y, ~ych, ~e; {{N}}: ~ym, ~, ~ym, ~ymi, ~ymi; {{Ms}}: ~ym, ~ej, ~ym, ~ych, ~ych; {{W}} ~y, ~a, ~e/~o, ~zy, ~e
